...
const Checkin = () => {
  ...
  useEffect(() => {
    const startFaceMatch = async (guestId, phoneNumber) => {
      ...
      try {
        ...
        console.log(`📸 [FACE_MATCH] Starting for ${tenDigitNumber}...`);

        const currentGuest = guests[index];
        const matchType = currentGuest.matchType;
        ...
        console.log("Original Reference Image:", refImage);
        ...
        showToast("error", error.message || "Face match failed");
  ...
  const handleVerifyGuest = async (index) => {
    ...
    try {
      ...
      // ================== ✅ FACE VERIFIED ==================
      if (plan === "enterprise" && rawStatus === "face_verified") {
        console.log("🔁 Face already verified → re-capture required");

        try {
          const aadhaarImage = await guestDetailsService.fetchGuestImage(
            countryCode,
            tenDigitNumber,
          );

          console.log("📸 Aadhaar Image (fresh):", aadhaarImage);
          ...
          setGuests((prev) => {
            ...
            newState[index] = {
              ...
              // ✅ store fresh reference image
              referenceImage: aadhaarImage || null,

              showWebcam: true,
              capturedImage: null,
              isMatching: false,
              matchType: "face_verified",
          ...
          console.error("❌ Failed to fetch Aadhaar image:", error);

          showToast("error", "Failed to fetch Aadhaar image");
