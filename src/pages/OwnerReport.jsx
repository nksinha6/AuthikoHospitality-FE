...
export default function OwnerReport() {
    ...
    const fetchGuestImage = async (guest) => {
        try {
            if (guest.image) {
                return guest.image.startsWith("data:") ? guest.image : `data:image/jpeg;base64,${guest.image}`;
            }
            ...
            if (phoneNumber) {
                const imageData = await guestDetailsService.fetchGuestImageWithRetry(countryCode, phoneNumber);
    ...
    const fetchGuestDetails = useCallback(
        async (showRefreshIndicator = false) => {
            ...
            try {
                ...
                // First set guests with basic data
                setGuests(transformedData);

                // Then fetch and enrich with property details if we have properties
                if (properties.length > 0) {
                    const enrichedData = transformedData.map(g => {
                        const propId = g._original?.propertyId || g._original?.PropertyId;
                        const property = properties.find(p => String(p.id) === String(propId));
                        return {
                            ...g,
                            propertyId: propId,
                            propertyName: property ? property.name : (g._original?.propertyName || "N/A"),
                            policeStation: property ? property.policeStation : (g._original?.policeStation || "-")
                        };
                    });
                    setGuests(enrichedData);
                }
            } catch (err) {
    ...
    const fetchAllProperties = useCallback(async () => {
        const propertyIds = userData?.propertyIds || [];
        ...
        try {
            const propertyPromises = propertyIds.map(id =>
                apiClient.get(API_ENDPOINTS.PROPERTY_BY_ID, { params: { propertyId: id } })
            ).filter(Boolean);

            const responses = await Promise.all(propertyPromises);
            const propertyData = responses.map(r => r.data).filter(Boolean);
    ...
    useEffect(() => {
        if (properties.length > 0) {
            fetchGuestDetails();
        } else if (userData?.propertyIds?.length === 0) {
            fetchGuestDetails();
        }
    }, [properties, fetchGuestDetails, userData]);

    /* ---------------- ENRICH GUESTS WHEN PROPERTIES CHANGE ---------------- */
    useEffect(() => {
        if (guests.length > 0 && properties.length > 0) {
    ...
    const applyAllFilters = useCallback(() => {
        ...
        if (selectedPropertyId !== "all") {
            data = data.filter(g => {
                const guestPropId = g.propertyId || g._original?.propertyId || g._original?.PropertyId;
                return String(guestPropId) === String(selectedPropertyId);
            });
        }

        // Name filter - search in firstName, lastName, and fullName
        ...
        // Booking ID filter
        if (filters.bookingId && filters.bookingId.trim()) {
            const q = filters.bookingId.toLowerCase().trim();
            data = data.filter((g) =>
        ...
        if (filters.state && filters.state.trim()) {
            const q = filters.state.toLowerCase().trim();
            data = data.filter((g) =>
        ...
        if (dateFilter?.selectedDate) {
            ...
            data = data.filter((g) => {
                const guestDate = g.date || g.checkInDate || g.checkInDateTime;
                return guestDate ? dayjs(guestDate).isSame(selected, "day") : false;
            });
        }

        setFilteredGuests(data);
    }, [guests, filters, dateFilter, selectedPropertyId]);
    ...
    const clearFilters = () => {
        setFilters({ name: "", bookingId: "", city: "", state: "" });
        setDateFilter(null);
    ...
    const isAllSelected = filteredGuests.length > 0 && selectedRows.length === filteredGuests.length;
    ...
    const handleDownloadSelectedPDF = async () => {
        ...
        try {
            ...
            const selectedGuestsData = filteredGuests.filter((g) => selectedRows.includes(g.bookingId));
            ...
            for (const guest of selectedGuestsData) {
                try {
                    ...
                    console.error(`Failed to fetch image for guest ${guest.bookingId}:`, error);
                }
            }
            ...
            selectedGuestsData.forEach((guest, guestIndex) => {
                ...
                addText("Property Owner Report", margin, 12, {
                    fontSize: 16,
                    fontStyle: "bold",
                    color: [255, 255, 255],
                });
                addText(`Property: ${guest.propertyName || "N/A"}`, margin, 20, {
                    fontSize: 10,
                    color: [200, 220, 210],
                });
                addText(`Booking ID: ${guest.bookingId}`, margin, 27, {
                    fontSize: 8,
                    color: [180, 200, 190],
                });
                ...
                addFieldLabel("Police Station", col1X, yPos + 27);
                addFieldValue(guest.policeStation || "-", col1X, yPos + 31);
                ...
                doc.setFont("helvetica", "normal");
                doc.text(
                    "Confidential - Property Owner Report",
                    margin,
                    pageHeight - 10,
                );
                doc.text(
            ...
            const dateTime = dayjs().format("YYYY-MM-DD_HH:mm").replace(":", "：");
            const fileName = `Property_Owner_Report_${dateTime}.pdf`;
    ...
    const dashboardStats = useMemo(() => {
        return {
            ...
            verifiedGuests: guests.filter(g => g.verificationStatus === "Verified").length,
            propertiesCount: properties.length || userData?.propertyIds?.length || 0
        };
    }, [guests, properties, userData]);

    /* ---------------- LOADING STATE ---------------- */
    if (isLoading && properties.length === 0 && userData?.propertyIds?.length > 0) {
        return (
                ...
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">
                            Consolidated overview for your properties
                        </p>
                    </div>
                </div>
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">
                        Consolidated overview for {dashboardStats.propertiesCount} properties
                    </p>
                </div>
                <button
            ...
            <div className="mb-6">
                <div className="grid grid-cols-4 gap-4">
                    <select
                        ...
                        className="w-full px-4 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1b3631] focus:border-[#1b3631] outline-none transition-all appearance-none"
                    >
                        ...
                        {(filters.name ||
                            ...
                            dateFilter?.selectedDate) && (
                                <button
                                    ...
                                    className="text-sm text-[#1b3631] hover:underline"
                                >
                                    Clear all filters
                                </button>
                            )}
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex align-center justify-end gap-3">
                    <button
                        onClick={() =>
                            exportToPDF({
                                fileName: "Property_Owner_Report",
            ...
            <UniversalTable
                ...
                format={{
                    ...
                    propertyName: (val) => <span className="font-semibold text-[#1b3631]">{val}</span>,
                    checkInDate: (_, row) => formatShortDate(row.date || row.checkInDate || row.checkInDateTime),
